...
public class LongitudinalFrameLabelManager : MonoBehaviour
{
    ...
    public void UpdateVisibility()
    {
        ...
        foreach (var item in transforms)
        {
            ...
            else
            {
                ...
                float distance = Mathf.Abs(item.transform.localPosition.z - lastTransfrom.localPosition.z);
    ...
    public void ReclibrateFrameLabel(FrameLabel frameLabel, Hullpart hullpart = null)
    {
        ...
        //frameLabel.size = (hullpart == null ? ShipBoundsComputer.Instance.shipBounds.size.x : hullpart.hullpartBounds.size.x) + sizeOffset; // Set the size as needed
    ...
    {
         return (hullpart == null ? ShipBoundsComputer.Instance.shipBounds.size.x :
            GroupingManager.Instance.vesselObject.GetCompartment(hullpart.owningCompartmentUid).
            compartmentMeshObjectReference.GetComponent<MeshRenderer>().bounds.size.x) + ServiceRunner.GetService<FrameLabelFeature>().data.sizeOffset; // Set the size as needed
    }

    public static Vector3 GetPosition(FrameLabel frameLabel, Hullpart hullpart)
    {
        Vector3 position;
        position.x = hullpart == null ? ShipBoundsComputer.Instance.shipBounds.center.x :
            GroupingManager.Instance.vesselObject.GetCompartment(hullpart.owningCompartmentUid).
            compartmentMeshObjectReference.GetComponent<MeshRenderer>().bounds.center.x;
        position.y = ServiceRunner.GetService<FrameLabelFeature>().data.yOffset;
        position.z = hullpart == null ? RelationBuilder.Instance.hullpartRegistry[frameLabel.gameObject.name].hullpartBoundsGlobal.center.z : hullpart.hullpartBounds.center.z;
        return position;
    }
    ...
    public void SpawnLabels()
    {
        ...
        {
            if (hullPart.Value.hullpartType.ToLower().Contains("long") || hullPart.Value.hullpartType.ToLower().Contains("centerbhd"))
            {
                Vector3 position;
                position.x = ShipBoundsComputer.Instance.shipBounds.center.x;
                position.y = ServiceRunner.GetService<FrameLabelFeature>().data.yOffset;
                position.z = hullPart.Value.hullpartBoundsGlobal.center.z;

                FrameLabel frameLabel = Instantiate(frameLable3D, parent);
                frameLabel.transform.rotation = Quaternion.LookRotation(Vector3.forward, Vector3.up);
                frameLabel.transform.position = position;
                frameLabel.label = Compartment.ConvertLine(hullPart.Value.name);
                frameLabel.size = (ShipBoundsComputer.Instance.shipBounds.size.x) + ServiceRunner.GetService<FrameLabelFeature>().data.sizeOffset; // Set the size as needed
                frameLabel.OnValidate(); // Call OnValidate to update the label
        ...
        transforms = transforms.OrderBy(t => t.transform.localPosition.z).ToList();
    }
}